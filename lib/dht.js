...
var Bucket = Class({ // See: http://www.bittorrent.org/beps/bep_0005.html#routing-table // third paragraph
    ...
    pull: function (index) { // remove and return a node from this bucket at (index)
        ...
        if (!this.length && !this.persist) this.table.splice(this.index, 1); // The splice methods returns the bucket but we don't need it anymore obviously
    ...
    push: function (node, force, assign, oldStamp) {
        ...
        if (this.length < K || force) { // The force option allows us to bypass the length limit
            ...
            return length; // Keep things ArrayLike
        }
    },
    sortBy: function (node) {
        ...
        return Array.prototype.sort.call(this, function (prev, next) { // We call the array sort method on this bucket, prev is initally the first node
            var a = cache[prev.id] || (cache[prev.id] = node.distance(prev)); // calc or returned the already calculated distance from the specified node argument which is usually our DHT node
...
var RoutingTable = Class({
    ...
    initialize: function initialize(node) { // node is this DHT node
        ...
        if (ss.storage.table && ss.storage.table.length) // if persistent table contains minified buckets.
            this.initWithStorage();
        //var observer = {'observe': this.toStorage.bind(this)}; // Commenting out because it was throwing an exception but it was working as is before, its reponsible saving the table when a user exits firefox
        ...
        if (!this.length) this.setBucketTimer(); // check bucket call setBucket timer but its not really necessary if there isn't any nodes in the table yet so its conditional
