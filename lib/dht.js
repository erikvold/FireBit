...
var Pipeline = Class({
    initialize: function initialize() {
        this.pipe = []; // Restore requests/response upto MAXIMUM_PIPELINE
    ...
    push: function (request, id) {
        ...
        if (this.pipe.length < MAXIMUM_PIPELINE) { // If not over the max
            if (this.sending && this.length < this.maxRequests) { // If not over the request limit
                //if (response) return request.send(); // The pipeline class is unstable at the moment fill free to work on it
                this.add(request, id); // add the request
            } else this.pipe.push(this.push.bind(this, request, id)); // If we reached the maximum amount of simultaneous request than bind this method with the original arguments and put that into the pipe
        }
    },
    pump: function () {
    ...
    add: function (request, id) {
        if (!Object.prototype.hasOwnProperty.call(this.requests, id)) { // The request object doesn't already contain a property with said ID
    ...
    remove: function (id, failure) { // only the setTimeout method should call this function with a second argument thus it serves as a boolean check
        if (Object.prototype.hasOwnProperty.call(this.requests, id)) { // If the requests object is storin the a request with that ID which should always be so this is a sanity check
    ...
    delay: function (request) {
        if (!this.pipe.length) request(); // If the pipe is empty then execute argument which should be a function
        else this.delayedPipe.push(request); // otherwise added it to the delayedPipe
    },
    start: function () {
        this.sending = true;
