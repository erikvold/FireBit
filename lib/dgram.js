 ...
 *     var socket = dgram.createSocket(function (data, fromAddr) {
 *          console.error('Received message: ' + data + '\nFrom: ' + fromAddr.address);
 *     });
 *     if (socket instanceof dgram.Socket) console.error('For example purposes...');
 *     socket.on('error', function (error) {
 *          console.exception(error);
 *     });
 *     socket.on('close', function (status) {
 *          console.error('Socket closed with ' + status + ' status);
 *     });
